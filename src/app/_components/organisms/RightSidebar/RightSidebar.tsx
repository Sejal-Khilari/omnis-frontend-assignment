...
const RightSidebar = () => {
  ...
  const getBadgeStyle = (status: string) => {
    return clsx("px-3 py-1 text-sm font-medium rounded-full text-center", {
  ...
  const getStatusStyle = (status: string) =>
    clsx("text-sm font-medium px-3 py-1 rounded-md inline-block", {
  ...
  const activities = [
    {
      ...
      description:
        "Vel rhoncus ullamcorper dui pellentesque sed morbi consequat.",
    },
    {
      date: "Feb 29, 2024 • 09:00 AM EST",
    ...
    <div className="w-100  border-l border-[#EDEAE3] p-6 space-y-6">
      ...
      <div>
        <Card className="p-4 flex justify-center items-center gap-3 border-[#E6EBF0]">
          ...
          <div className="text-sm font-medium text-gray-800">898-989-8898</div>
        </Card>
      </div>
      {/* Progress Section */}
        ...
        <div className="space-y-5 pt-2">
          {records.map((record, index) => (
            ...
            >
              ...
              <span className={getBadgeStyle(record.status)}>
                {record.status}
              </span>
            </div>
          ))}
